...
class SnetRecognizer:
    ...
    def nltk_recognizer(self, input_text):
        """ NLTK Named Entity Recognizer  Option

        :param input_text:
        :return:
        """
        result = self.structure_ne(self.nltk_tagger(self.word_tokenize(input_text)))
        # logger.debug("nltk input text => ", str(result))
        return result

    def stanford_recognizer(self, input_text):
        """ Stanford Named Entity Recognizer Option

        :param input_text:
        :return:
        """

        result = self.structure_ne(self.stanford_tree(self.bio_tagger(self.stanford_tagger(self.word_tokenize(input_text)))))
        return result
